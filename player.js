function createPlayer({
  ...
  (function activatePlayButtons(){
    ...
    $pauseButton.off('click').on('click', () => {
      player.pause();
      showPlay();
    });
  ...
  (function activateVolumeButtons(){
    ...
    $volumeButton.off('click').on('click', () => {
      player.setVolume(0);
      updateVolumeButtons();
    });
    ...
    player.on(Playable.VIDEO_EVENTS.VOLUME_CHANGED, updateVolumeButtons);
    updateVolumeButtons();
  })();
  ...
  (function activateTimeLabels(){
    ...
    const $overlayCurrent = $playerContainer.find('.js-overlay-current');
    function showCurrentTime(){
      const totalSeconds = player.getCurrentTime();
      const formattedTime = formatTime(totalSeconds);
      $currentTime.text(formattedTime);
      if ($overlayCurrent.length){
        $overlayCurrent.text(formattedTime);
      }
    }
  ...
  (function activateProgressbar(){
    ...
    const $knob = $progress.find('.js-progress-knob');
    ...
    function updateSliderWidth(){
      ...
      if (!durationSeconds){
        $slider.css("width", "0%");
        $progress.find('.js-progress-knob').css('left', `0%`);
        return;
      }
